...
from settings import CORNER_CELLS, EPISODES_FOR_TRAINING_Q, MIN_EPSILON_Q, MAX_EPSILON_Q
...
class QLearningAgent:
    ...
    def calculate_reward(self, current_state, action, next_state):
        ...
        if self.agent_type == 'player':
            own_goal = player_goal
            enemy_goal = agent_goal
            current_entity_position = current_state['player_position']
            next_entity_position = next_state['player_position']
            own_corners = player_corners
            enemy_corners = agent_corners
        else:
            own_goal = agent_goal
        ...
        if next_state['ball_position'] == enemy_goal or next_state['ball_position'] in enemy_corners:
            reward += 100
        elif next_state['ball_position'] == own_goal or next_state['ball_position'] in own_corners:
            reward -= 100
    ...
    def choose_action(self, state):
        ...
        if random.uniform(0, 1) < self.calculate_epsilon(self.scoreboard.left_score + self.scoreboard.right_score, EPISODES_FOR_TRAINING_Q, MIN_EPSILON_Q, MAX_EPSILON_Q):
            ...
            return max(sap_values, key=sap_values.get) if sap_values else random.choice(possible_actions)
