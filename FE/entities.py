import pygame
import random
from settings import GOAL_CELLS
...
class Player:
    ...
    def highlight_moves(self, screen, agent, ball):
        for col, row in neighbor_hex(self.column, self.row):
            ...
            if (col, row) != (agent.column, agent.row) and (col, row) != (ball.column, ball.row) and (col, row) not in GOAL_CELLS:
                highlight_hex(screen, x, y, (173, 216, 230, 128))
            if (col, row) != (agent.column, agent.row) and (col, row) == (ball.column, ball.row) and (col, row) not in GOAL_CELLS and not ball.selected:
                highlight_hex(screen, x, y, (0, 255, 0, 128))

    def move_to(self, column, row):
        if (column, row) not in GOAL_CELLS:
            self.column = column
            self.row = row
            self.x, self.y = hex_center(self.column, self.row)
            self.selected = False 
    ...
    def get_possible_actions(self, agent, ball):
        ...
        valid_moves = [
            ...
            if cell not in GOAL_CELLS and cell != ball_position and cell != agent_position
        ]
...
class Agent:
    ...
    def get_possible_actions(self, player, ball):
        ...
        for cell in neighbors:
            if cell not in GOAL_CELLS and cell != ball_position and cell != player_position:
