...
def log_to_csv(entity, state, action, filename='states_no_rewards.csv'):
    fieldnames = ['entity', 'ball_position', 'player_position', 
                  'agent_position', 'score', 'action']
...
def print_successful_action_percentages(
    fallback_player_qlearning_count,
    successful_player_qlearning_count,
    fallback_agent_qlearning_count,
    successful_agent_qlearning_count,
    fallback_minimax_count,
    successful_minimax_count,
    fallback_belief_count,
    successful_belief_count
):
    def calculate_percentage(successful_count, fallback_count):
        total_count = successful_count + fallback_count
        if total_count == 0:
            return 0.0
        return (successful_count / total_count) * 100

    player_qlearning_success_percentage = calculate_percentage(successful_player_qlearning_count, fallback_player_qlearning_count)
    agent_qlearning_success_percentage = calculate_percentage(successful_agent_qlearning_count, fallback_agent_qlearning_count)
    minimax_success_percentage = calculate_percentage(successful_minimax_count, fallback_minimax_count)
    belief_success_percentage = calculate_percentage(successful_belief_count, fallback_belief_count)

    print(f"Player Q-Learning Success Percentage: {player_qlearning_success_percentage:.2f}%")
    print(f"Agent Q-Learning Success Percentage: {agent_qlearning_success_percentage:.2f}%")
    print(f"Minimax Q-Learning Success Percentage: {minimax_success_percentage:.2f}%")
    print(f"Belief-Based Q-Learning Success Percentage: {belief_success_percentage:.2f}%")

def modify_state_for_small_env(state):
    # Extract the x-coordinate of the ball
    ball_x = state['ball_position'][0]
    
    # Determine the offset based on the x-coordinate of the ball
    if ball_x in [0, 1, 2, 3]:
        offset = 0
    elif ball_x in [4, 5, 6]:
        offset = 2
    elif ball_x in [7, 8, 9, 10]:
        offset = 4
    else:
        raise ValueError(f"Unexpected ball x-coordinate: {ball_x}")

    # Apply the offset to all positions
    modified_state = {
        'agent_position': (state['agent_position'][0] - offset, state['agent_position'][1]),
        'ball_position': (state['ball_position'][0] - offset, state['ball_position'][1]),
        'player_position': (state['player_position'][0] - offset, state['player_position'][1])
    }
